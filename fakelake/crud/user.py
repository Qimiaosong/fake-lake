...
def create(userInfo: UserSchema):
    logger.info(f"current create user, param is: {userInfo.dict()}")
    user_name = f"lake-{userInfo.name}"
    user = models.UserModel(
        name=userInfo.name,
        owner_id=userInfo.owner_id,
        pg_password=CommonUtil.generate_secret_password(generate_password(), _settings.db.secret_key),
        extra={CommonVars.REAL_NAME: user_name}
    )
    uwrs = []
    for wp in userInfo.workspaces:
        ...
        if 0 in cur_roles.keys() or 1 in cur_roles.keys():
            is_admin = True
        # 调用不同的认证模块的方法来创建用户，授予权限
        AuthPg.create_grant_user(wp_name, user_name, user.pg_password, is_admin)
        AuthUnix.create_user(wp_name, user_name, user.pg_password)
        if global_settings().compute_engine.mode == ComputeEngineType.HADOOP:
            AuthHadoop.create_user(groupname=wp_name, username=user_name, password=user.pg_password, hosts="all")
            AuthUnix.copy_users()
        if len(cur_roles) > 1:
            cur_role = next(iter(sorted(cur_roles.items())))[1]
            for mod, pers in cur_role.data_permission.items():
                # 如果角色数量大于1，则选择其中一个角色，遍历数据权限，调用add_policy来添加策略
                AuthCasbin().add_policy(user_name,
                                        workspace_name=wp_name,
                                        role=cur_role.name,
                                        module_name=mod,
